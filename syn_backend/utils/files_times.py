...
def generate_schedule_time_next_day(total_videos, videos_per_day = 1, daily_times=None, timestamps=False, start_days=0):
    ...
    if len(daily_times) < videos_per_day:
        # Duplicate provided slots so we have enough timestamps for all videos_per_day.
        extended = []
        while len(extended) < videos_per_day:
            extended.append(daily_times[len(extended) % len(daily_times)])
        daily_times = extended
