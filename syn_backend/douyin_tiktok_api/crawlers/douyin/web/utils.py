...
def _build_httpx_client(**kwargs):
    try:
...
def _build_httpx_async_client(**kwargs):
    try:
...
class TokenManager:
    ...
    def gen_real_msToken(cls) -> str:
        ...
        with _build_httpx_client(transport=transport, proxies=cls.proxies, timeout=10.0) as client:
            max_attempts = 3
                ...
                try:
                    ...
                    if response.status_code in {429, 500, 502, 503, 504}:
                        if attempt < max_attempts:
                            ...
                            continue
                        logger.warning(
                            "Douyin msToken API 临时不可用（{0}），将使用虚假 msToken。"
                    ...
                    if len(msToken) not in [120, 128]:
                        logger.warning(
                            "Douyin msToken 返回长度异常：{0}，将使用虚假 msToken。".format(msToken)
                        )
                        return cls.gen_false_msToken()
...
class SecUserIdFetcher:
    ...
    async def get_sec_user_id(cls, url: str) -> str:
        ...
        try:
            ...
            async with _build_httpx_async_client(
