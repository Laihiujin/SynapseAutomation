...
说明：
- 当前复用旧实现 `uploader/ks_uploader/main.py`
"""
...
class KuaishouUpload(BasePlatform):
    def __init__(self):
        super().__init__(platform_code=4, platform_name="快手")
        ...
        if publish_date:
            ...
            if publish_value and isinstance(publish_value, datetime):
                ...
                if time_diff < min_delay:
                    raise ValueError(
                        f"快手定时发布时间必须在1小时后，当前距离: {int(time_diff / 60)}分钟"
                    )
                if time_diff > max_delay:
                    raise ValueError(
                        f"快手定时发布时间不能超过14天，当前距离: {int(time_diff / 86400)}天"
                    )

        # 快手只有一个输入框，_fill_title_and_topics 会自动组合：标题 + \n + 标签
        # 不需要在这里拼接 description
        uploader = KSVideo(title, file_path, tags or [], publish_value, account_file)
        await uploader.main()

        return {"success": True, "message": "上传成功"}


kuaishou_upload = KuaishouUpload()
