...
async def main():
    ...
    # The logic for handling cookies has been moved to the login functions in myUtils/login.py
    # and is managed by the CookieManager.
    ...
    if args.action == 'login':
        ...
        cookie = cookie_manager.get_cookie(args.platform, args.account_name)
        if not cookie:
            print(f"Could not find cookie for account {args.account_name} on platform {args.platform}. Please login first.")
            exit()

        if args.platform == SOCIAL_MEDIA_DOUYIN:
            app = DouYinVideo(title, video_file, tags, publish_date, cookie)
        elif args.platform == SOCIAL_MEDIA_TIKTOK:
            app = TiktokVideo(title, video_file, tags, publish_date, cookie)
        elif args.platform == SOCIAL_MEDIA_TENCENT:
            category = TencentZoneTypes.LIFESTYLE.value  # 标记原创需要否则不需要传
            app = TencentVideo(title, video_file, tags, publish_date, cookie, category)
        elif args.platform == SOCIAL_MEDIA_KUAISHOU:
            app = KSVideo(title, video_file, tags, publish_date, cookie)
        else:
